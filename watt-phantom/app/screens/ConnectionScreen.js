...
export default function ConnectionScreen({ navigation }) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>WattPhantom</Text>
      <Text style={styles.subtitle}>USB-C PD Manipulation & Covert Channel</Text>
      ...
      <TouchableOpacity style={styles.button} onPress={handleScan} disabled={scanning}>
        <Text style={styles.buttonText}>{scanning ? 'Scanning...' : 'Scan for WattPhantom'}</Text>
      </TouchableOpacity>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.deviceItem} onPress={() => handleConnect(item)}>
            <Text style={styles.deviceName}>{item.name || 'WattPhantom'}</Text>
      ...
      <Text style={styles.disclaimer}>
        ...
        Manipulating USB-C PD on unauthorized devices may cause damage and
        may be illegal. The author (jayis1) assumes no liability for misuse.
      </Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 28, fontWeight: 'bold', color: '#00d4aa', marginTop: 20 },
  ...
  button: { backgroundColor: '#00d4aa', padding: 14, borderRadius: 8, alignItems: 'center', marginBottom: 16 },
  ...
  deviceRssi: { color: '#6e7681', fontSize: 11 },
  empty: { color: '#6e7681', textAlign: 'center', marginTop: 20 },
  disclaimer: { color: '#f85149', fontSize: 11, marginTop: 20, textAlign: 'center' },
});